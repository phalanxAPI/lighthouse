...
export const getAPIInfo = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const apiInfoWithHits = await Promise.all(apiInfo.map(async (api) => {
            const hits = await RequestLog.countDocuments({ apiId: api._id, requestType: 'INCOMING' });
            return {
                ...api.toObject(),
                hits
            };
...
export const getAPIInfoById = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const hits = await RequestLog.countDocuments({ apiId: apiObjectId, requestType: 'INCOMING' });
...
export const getRequestLogsForGraph = async (req: Request, res: Response) => {
    ...
    try {
        ...
        const logs = await RequestLog.aggregate([
            ...
            {
                $sort: { "_id.serverId": 1, "_id.year": 1, "_id.month": 1, "_id.day": 1, "_id.hour": 1 }
            }
        ]);
        ...
        res.status(500).json({ message: 'Error fetching request logs for graph', error });
    }
};
...
export const getRequestLogsForGraphByApiId = async (req: Request, res: Response) => {
    const { apiId } = req.query;
    ...
    try {
        ...
        res.status(500).json({ message: 'Error fetching request logs for API on servers', error });
    }
};